...
export default function Contacto() {
  ...
  return (
    <motion.div key="contacto" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.3 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px', marginBottom: '32px' }}>
        ...
        <div style={{ background: 'var(--color-bg)', border: '0.5px solid var(--color-border)', borderRadius: 'var(--radius-lg)', padding: '24px' }}>
          ...
          {[
            { icon: '🐦', name: 'Twitter / X', handle: '@FindYourBet', url: '#' },
            { icon: '📸', name: 'Instagram', handle: '@findyourbet', url: '#' },
            { icon: '💬', name: 'Telegram', handle: 't.me/findyourbet', url: '#' },
          ].map((s, i) => (
            <a key={i} href={s.url}
      ...
      <div style={{ background: 'var(--color-bg)', border: '0.5px solid var(--color-border)', borderRadius: 'var(--radius-lg)', padding: '24px', maxWidth: '560px' }}>
        <div style={{ fontSize: '15px', fontWeight: 600, marginBottom: '20px' }}>✉️ Envíanos un mensaje</div>

        {sent ? (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
            style={{ textAlign: 'center', padding: '32px', color: 'var(--color-primary)' }}>
            <div style={{ fontSize: '40px', marginBottom: '12px' }}>✅</div>
            <div style={{ fontWeight: 600, fontSize: '16px' }}>¡Mensaje enviado!</div>
            <div style={{ fontSize: '13px', color: 'var(--color-text-muted)', marginTop: '8px' }}>Te responderemos en menos de 24h.</div>
          </motion.div>
        ) : (
          <>
            ...
            <div style={{ marginBottom: '20px' }}>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 600, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '8px' }}>Mensaje</label>
              <textarea style={{ ...inputStyle, resize: 'vertical' }} rows={4} placeholder="¿En qué podemos ayudarte?" value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} />
            </div>
